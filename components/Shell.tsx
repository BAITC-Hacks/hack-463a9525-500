...
import { Bell, Bot, CalendarDays, LayoutGrid, ScanLine, Search, Settings2, UserRound, ChartNoAxesCombined } from "lucide-react";
...
export default function Shell({ children }: { children: React.ReactNode }) {
  ...
  const pageName = pathname === "/" ? "Обзор сети" : pathname === "/network" ? "Граф переводов" :
    ...
    pathname === "/methodology" ? "Методика" : pathname === "/analyst" ? "Аналитик" : "MoneyGraph";
  return <div className="outer-stage"><div className="dashboard-shell">
    <aside className="side-rail" aria-label="Основная навигация">
      <Link className="logo-box" href="/" aria-label="Граф денег, главная"><Image src="/brand/moneygraph-logo.png" width={50} height={50} alt="" priority /></Link>
    ...
    <div className="main-area">
      <header className="top-header"><div className="top-label">{pageName}</div><div className="top-actions">
        ...
        <div className="profile-chip"><span className="profile-avatar">A</span><span><strong>AML Analyst</strong><small>Локальная сессия</small></span></div>
      </div></header>
      {children}
    </div>
  </div></div>;
