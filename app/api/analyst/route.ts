export async function POST(request: Request) {
  ...
  if (history !== undefined && (!Array.isArray(history) || history.length > 8 || history.some(item => !item || typeof item !== "object" || !["user", "assistant"].includes(item.role) || typeof item.content !== "string" || item.content.length > 2000))) {
  ...
  try {
    const response = await fetch(`${process.env.BACKEND_URL ?? "http://127.0.0.1:8000"}/api/analyst`, {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ question, history: history ?? [] }), cache: "no-store"
    });
