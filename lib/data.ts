...
export type GraphNode = {
  ...
  weighted_pagerank: number;
  betweenness_centrality: number;
  cluster_bridge_score: number;
  similar_amount_rapid_share: number;
  anomaly_score: number;
};

export type GraphEdge = { src: string; dst: string; sum_kzt: number; n_tx: number };
export type Cluster = { cluster_id: number; n_nodes: number; n_seed: number; sum_kzt_internal: number; incoming_external_volume: number; outgoing_external_volume: number; top_gids: string; role_distribution: string; hypothesis: string };
